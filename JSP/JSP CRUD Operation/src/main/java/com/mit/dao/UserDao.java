...
public static int save(User u){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into register1(name,password,email,sex,country) values(?,?,?,?,?)");
...
public static int update(User u){
	...
	try{
		Connection con=getConnection();
		PreparedStatement ps=con.prepareStatement("update register1 set name=?,password=?,email=?,sex=?,country=? where id=?");
		ps.setString(1,u.getName());
		ps.setString(2,u.getPassword());
		ps.setString(3,u.getEmail());
		ps.setString(4,u.getSex());
